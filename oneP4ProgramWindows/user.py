...
try:
    ...
    os.remove("./P4/test-case1/packets_in.pcap")
    os.remove("./P4/test-case1/packets_out.pcap")
        ...
        for packet in packets:
            if QuizHeaderReply in packet:
                ...
                if (correct_reply == 1):
                    ...
                    print(f"You have failed, keep playing.")
                else:
                    print(f"There was a problem" )
                    
                print(f"The capital of {question_text_reply} is {correct_text_reply}")
    else:
    # Handle the case when the timeout is reached
        print("File not created within the specified timeout.")
        exit()
